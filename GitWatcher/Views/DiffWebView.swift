...
//  GitWatcher
//
//  diff 뷰어. 웜 WKWebView 인스턴스 하나를 띄워두고 Swift→JS 로 unified diff 텍스트만
//  주입한다(파일마다 새 웹뷰 금지, 웹뷰가 직접 fetch 하지 않음).
//  diff2html 동급의 경량 렌더러를 HTML 문자열로 내장 — 네트워크/CDN 의존 없음(로컬 전용 원칙).
//
...
struct DiffWebView: NSViewRepresentable {
    /// unified diff 텍스트(git diff / git show 출력). 빈 문자열이면 빈 상태.
    let diffText: String
    ...
    func updateNSView(_ nsView: WKWebView, context: Context) {
        context.coordinator.render(diffText)
    }
    ...
    final class Coordinator: NSObject, WKNavigationDelegate {
        ...
        func render(_ diff: String) {
            guard diff != lastRendered else { return }
            lastRendered = diff
            if loaded { inject(diff) } else { pending = diff }
        }
        ...
        private func inject(_ diff: String) {
            guard let webView else { return }
            // 문자열을 JS 리터럴로 안전 변환(인젝션/이스케이프 처리).
            let data = (try? JSONSerialization.data(withJSONObject: diff, options: [.fragmentsAllowed])) ?? Data("\"\"".utf8)
            let literal = String(data: data, encoding: .utf8) ?? "\"\""
            webView.evaluateJavaScript("window.renderDiff(\(literal));", completionHandler: nil)
        }
    }
    ...
    <script>
      // unified diff 텍스트를 라인별로 분류해 표로 렌더(side-by-side 아님, unified view).
      window.renderDiff = function(text) {
        const empty = document.getElementById('empty');
        const table = document.getElementById('diff');
        if (!text || !text.trim()) {
          empty.style.display = ''; table.style.display = 'none'; table.innerHTML = '';
          return;
        }
        ...
        for (const line of lines) {
          ...
          if (line.startsWith('+')) {
            html += row('add', '', newLn++, body);
          } else if (line.startsWith('-')) {
            html += row('del', oldLn++, '', body);
          } else {
            html += row('ctx', oldLn++, newLn++, line.startsWith(' ') ? body : line);
          }
        }
